...
/**
  ...
  */
class Ellipse(center: Point, xRadius: Double, yRadius: Double, angle: Int) {

  /**
    * Creates an ellipse with its radius's angled along their axis.
    */
  def this(center: Point, xRadius: Double, yRadius: Double) = this(center, xRadius, yRadius, 0)
  ...
  /**
    ...
    */
  def intersectsLine(lStart: Point, lStop: Point): Option[Seq[DPoint]] = {
    val x1 = center.x.toDouble
    val y1 = center.y.toDouble
    val u1 = lStart.x
    val v1 = lStart.y
    val u2 = lStop.x
    val v2 = lStop.y
    val dx = u2 - u1
    val dy = v2 - v1
    val q0 = k1 * SQR(u1 - x1) + k2 * (u1 - x1) * (v1 - y1) + k3 * SQR(v1 - y1) - 1
    val q1 = (2 * k1 * dx * (u1 - x1)) + (k2 * dx * (v1 - y1)) + (k2 * dy * (u1 - x1)) + (2 * k3 * dy * (v1 - y1))
    val q2 = (k1 * SQR(dx)) + (k2 * dx * dy) + (k3 * SQR(dy))

    val d = SQR(q1) - (4 * q0 * q2)
 ...
 *
    ...
    */
  def intersectsRectangle(rStart: Point, rStop: Point): IntersectCase = {
    ...
    if (intersectsLine(Point(rStart.x, rStop.y), rStop).isDefined) { // Top
      Intersects

    } else if (intersectsLine(Point(rStart.x, rStop.y), rStart).isDefined) { // Left
      Intersects

    } else if (intersectsLine(rStart, Point(rStop.x, rStart.y)).isDefined) { // Bottom
      Intersects

    } else if (intersectsLine(rStop, Point(rStop.x, rStart.y)).isDefined) { // Right
      Intersects
